...
└── tests/                          # 自动化单元与集成测试套件
    └── test_engines.py             # 核心引擎全流程检验测试 (覆盖 41 项严密测试/自适应四叉树/闰年保真/栅格引擎/闭合性)
```
...
* **[算法突破] 自适应潮位控制网格 (Adaptive Tide Control Grid)**：攻克 10m/30m 沿海 DEM 全像元逐时刻解算引发的物理算力爆炸难题，采用自适应四叉树细分（Adaptive Quadtree）与阻隔水体拓扑连通防护（Topology Guard），在有效水域/潮滩提取稀疏自适应控制网格（默认 4km）长时序，结合反距离权重 (IDW) 空间插值与高效互补累积分布 (CCDF)，高效解算整年潜在天文潮淹没频率（0% ~ 100%）空间栅格；
...
* **[自动化测试全面扩展至 41 项]**：新增四叉树动态细分节点增长、最小步长终止与质量位 32、阻隔水体拓扑连通隔离、官方掩膜五类规范值、投影坐标系重投影、经度圆周跨界 (0°/180°)、投影米/英尺单位自适应转换与端到端离线预言机验证，全部通过 (41/41 OK)。
