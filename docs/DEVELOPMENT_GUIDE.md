...
版本: v1.6 | 维护者: 王宇浩 (Yuhao Wang) | 机构领域: 沿海海洋动力学与大地测量学 (Coastal Ocean Dynamics & Geodesy)

---

## 一、 双语注释与代码规范 / Bilingual Code & Comment Standards

CoastTideX 作为一个具备国际学术水准的开源科研工程系统，要求所有核心算法、数据结构、公开接口与模块文档必须遵循**中英双语 (Chinese & English)** 规范。

### 1. 模块级文档字符串 (Module Docstring)
- 每个 `.py` 核心文件顶部必须包含中英双语模块说明、核心科学定义、理论公式与内存管理不变量。
- 必须明确注明模块的数学解析基础与质量控制 (QC) 位掩膜定义。

### 2. 公共函数与类说明 (Function & Class Docstrings)
...
1. **严禁在内存中创建像元全时空 3D 矩阵 (No Full 3D Array)**：
   - 栅格影像像元数常达 $10^7 \sim 10^8$，年时步常达 $17,568$。创建 $P 	imes T$ 矩阵将直接引发数百 GB 内存崩溃 (OOM)。
   - 必须采用**空间二维分块 (Window Blocks, 512x512)** 与**时间维流式批次 (Time Chunks, 1000步)** 双重循环流式累加。
2. **控制节点数量预算控制**：
